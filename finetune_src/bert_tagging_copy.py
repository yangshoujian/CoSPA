...
class BertTagging:
    ...
    def create_model(self, input_ids, input_mask, segment_ids, stroke_ids, lmask, labels, batch_size, py_labels, is_training):
        ...
        with tf.variable_scope('bert', reuse=tf.AUTO_REUSE):
            ...
            # CopyNet
            seq_embedding = model.embedding_output
            embedding_size = seq_embedding[-1].shape[-1].value
        ...
        with tf.variable_scope("loss", reuse=tf.AUTO_REUSE):
            ...
            # CopyNet
            copy_weights=tf.get_variable("copy_weights",[1,embedding_size],initializer=tf.truncated_normal_initializer(stddev=0.02))
            copy_bias=tf.get_variable("copy_bias",[180],initializer=tf.truncated_normal_initializer(stddev=0.02))
            ...
            """
            # CopyNet
            copy_logits=tf.matmul(seq_embedding,copy_weights,transpose_b=True)
            copy_logits= tf.squeeze(copy_logits, axis=-1)
            copy_logits=tf.nn.bias_add(copy_logits,copy_bias)
            probabilities_copy=tf.nn.sigmoid(copy_logits)
            probabilities_copy = tf.expand_dims(probabilities_copy, axis=-1)
            probabilities_copy=tf.reshape(probabilities_copy,[-1,1])
            helper_tensor=tf.ones([1, self.num_class],dtype=tf.float32)
            probabilities_copy=tf.matmul(probabilities_copy,helper_tensor)
            one_hot_labels = tf.one_hot(labels, depth=self.num_class, dtype=tf.float32)
            probabilities=probabilities_copy*one_hot_labels + (1-probabilities_copy)*probabilities
            """

            # Copy_prob_diff
            temperature = 5.0
            _input_ids = tf.reshape(input_ids, shape=(1, -1))
            _input_ids = tf.squeeze(_input_ids)
            indices = tf.stack([tf.range(batch_size * self.MAX_SEN_LEN), _input_ids])
            indices = tf.transpose(indices)
            in_prob = tf.gather_nd(probabilities, indices)
            top_prob = tf.reduce_max(probabilities, axis=-1)
            diff_prob = top_prob - in_prob
            copy_logits=tf.matmul(seq_embedding,copy_weights,transpose_b=True)
            copy_logits= tf.squeeze(copy_logits, axis=-1)
            copy_logits=tf.nn.bias_add(copy_logits,copy_bias)
            probabilities_copy=tf.nn.sigmoid(tf.nn.relu(copy_logits))
            probabilities_copy=tf.reshape(probabilities_copy,[-1])
            probabilities_copy = probabilities_copy / tf.exp(temperature * diff_prob)
            probabilities_copy = tf.expand_dims(probabilities_copy, axis=-1)
            helper_tensor=tf.ones([1, self.num_class],dtype=tf.float32)
            probabilities_copy=tf.matmul(probabilities_copy,helper_tensor)
            one_hot_labels = tf.one_hot(labels, depth=self.num_class, dtype=tf.float32)
