...
from bert_tagging_copy import DataProcessor, BertTagging
import modeling
import optimization
import time
...
def evaluate(FLAGS, sess, model, data_processor, label_list=None):
    ...
    (pred_loss, pred_probs, gold_probs, gold_mask, py_probs, py_one_hot_labels, fusion_prob) = \
               model.create_model(input_ids, input_mask, segment_ids, stroke_ids, lmask, label_ids, batch_size, py_labels, is_training=False)
...
def train(FLAGS):
    ...
    (loss, probs, golds, _, py_probs, py_one_hot_labels, fusion_prob) = model.create_model(input_ids, input_mask, segment_ids, stroke_ids, lmask, label_ids, batch_size, py_labels, is_training=True)
